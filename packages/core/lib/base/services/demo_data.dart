...
class DemoData {
  ...
  static const _vehicles = [
    ...
    {
      'id': 'demo-vehicle-6',
      'license_plate': 'B-2468-DEM',
      'fleet_group_id': 'demo-fleet-group',
      'fleet_group_name': 'PT. Demo Logistik',
      'vehicle_category': 'TRUCK',
      'vehicle_activity': 'TEMP_SILENCE',
      'in_operation': false,
      'ignition': 0,
      'speed': 0,
      'direction': 120,
  ...
  static Iterable<Map<String, dynamic>> _byActivity(String activity) {
    // MonitoringApi.activityParam sekarang mengirim kode huruf kecil
    // (in_operation, moving, ...) mengikuti server; dinormalisasi di sini
    // supaya cocok dengan vehicle_activity dummy yang huruf besar.
    final code = activity.trim().toUpperCase();
    if (code.isEmpty) return _vehicles;
    if (code == 'IN_OPERATION') {
      return _vehicles.where((v) => v['in_operation'] == true);
    }
    return _vehicles.where((v) => v['vehicle_activity'] == code);
  }
    ...
    return {
      ...
      'metadata': {
        ...
        'summary': {
          ...
          'TEMP_SILENCE': countBy(
            (v) => v['vehicle_activity'] == 'TEMP_SILENCE',
          ),
